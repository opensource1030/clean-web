        ...
        <modal v-if="$store.getters['error/hasError']" @close="$store.dispatch('error/clearAll')">
          ...
          <div class="small-12 columns titles">
            ...
            <div class="medium-6 columns presetname">
              <label>Title
                <input id="name" type="text" placeholder="" :value="preset.name" @input="$store.commit('preset/updateField',{e:$event,type:'name'})"  >
              </label>
            </div>
                  ...
                  <div class="accordion-content variations"  data-tab-content   >
                    <div class="row">
                      ...
                      <div class="crop" v-for="(v,index) in variations">
                        <input type="checkbox" :id="'variation-' + v.id" name="variations" :value="v.id" :checked="v.checks"  @change="$store.commit('preset/updateVariations',{e:$event,type:'variations',i:index})" >
